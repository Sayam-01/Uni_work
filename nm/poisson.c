...
{
    int i, j, m, n, iter = 0;
    float u[10][10], u_old[10][10], f[10][10], tol, max_diff, h = 1.0;
    ...
    // Input source term f(i,j) for interior points
    printf("Enter source term f(i,j) for interior points:\n");
    for(i = 1; i < m-1; i++)
        for(j = 1; j < n-1; j++)
            scanf("%f", &f[i][j]);
